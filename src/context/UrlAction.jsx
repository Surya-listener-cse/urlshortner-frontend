...
function UrlAction(props) {
  ...
  const getUrls = async (token) => {
    try {
      ...
      const res = await axios.get(`https://urlshortner-backend-x4um.onrender.com/get_url`, config);
      dispatch({
        type: "GET_URLS",
        payload: [...res.data],
      });
  ...
  const ShortenUrls = async (url,token) => {
    try {
      ...
      const res = await axios.post(
        `https://urlshortner-backend-x4um.onrender.com/shorten_url`,
        url,
        config
      );
      dispatch({
  ...
  const editUrl = async (selectedUrl,token) => {
    try {
      ...
      const res = await axios.put(
        `https://urlshortner-backend-x4um.onrender.com/edit_shortner`,
        selectedUrl,
        config
      );
      // console.log(res);
  ...
  const deleteUrls = async (id,token) => {
    try {
      ...
      const res = await axios.delete(
        `https://urlshortner-backend-x4um.onrender.com/delete_shortner/${id}`,
        config
      );
  ...
  const getViewCount = async () => {
    try {
      setLoading("SET_LOADING");
      const res = await axios.get(`https://urlshortner-backend-x4um.onrender.com/get_viewcount`);
      dispatch({
        type: "GET_VIEWCOUNT",
        payload: res.data,
      });
